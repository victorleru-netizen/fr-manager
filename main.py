...
# ─── LE FORMULAIRE DE RETOUR (5 CHAMPS PARFAITS) ───
class DetailAvisModal(discord.ui.Modal):
    def __init__(self, staff_member: discord.Member):
        ...
        self.prof_input = discord.ui.TextInput(label="Professionnalisme (Note de 1 à 5)", placeholder="Sérieux et respect des procédures...", required=True, min_length=1, max_length=1)
        self.symp_input = discord.ui.TextInput(label="Sympathie & Accueil (Note de 1 à 5)", placeholder="Gentillesse et accueil...", required=True, min_length=1, max_length=1)
        self.rap_input = discord.ui.TextInput(label="Rapidité (Note de 1 à 5)", placeholder="Prise en charge et efficacité...", required=True, min_length=1, max_length=1)
        self.ecoute_input = discord.ui.TextInput(label="Écoute (Note de 1 à 5)", placeholder="Patience et compréhension...", required=True, min_length=1, max_length=1)
        self.comm_input = discord.ui.TextInput(label="Commentaire général", style=discord.TextStyle.long, placeholder="Exprimez-vous sur votre expérience avec ce staff...", required=True, max_length=300)
...
# ─── SELECTION ET LOGIQUE DES COOLDOWNS ───
...
# ─── PANNEAUX DE CONFIGURATION (ADMIN) ───
...
# ─── COMMANDES COMPLÈTES ADDITIONNELLES ───
...
async def avis_blacklist(interaction: discord.Interaction, action: str, membre: discord.Member):
    ...
    if action == "add":
        if m_id not in srv_cfg["blacklist"]:
            ...
            save_config(config_global)
            await interaction.response.send_message(f"✅ {membre.mention} a été ajouté à la blacklist.", ephemeral=True)
        else:
